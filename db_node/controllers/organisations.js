...
exports.updateBudgets = async (req, res, next) => {
    ...
    /* if(category == "corporation"){
        ...
        console.log(private_funds);
        await db.query(`UPDATE corporation SET private_funds = $2 WHERE org_name = $1`, [org_name, private_funds]);
    }
    else if (category == "university"){
        public_funds = req.body.public_funds;
        console.log(public_funds);
        await db.query("UPDATE university SET public_funds = $1 WHERE org_name = $2", [public_funds,org_name]);
    }
    else{
        private_funds = req.body.private_funds;
        public_funds = req.body.public_funds;
        await db.query("UPDATE research_center SET private_funds = $2 , public_funds = $3 WHERE org_name = $1", [org_name, private_funds,public_funds]);
